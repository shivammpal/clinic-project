...
type DoctorProfilePageProps = {
  doctorId: string; // The ID of the doctor to display
};
  ...
  useEffect(() => {
    ...
    const fetchDoctorProfile = async () => {
      try {
        ...
        setError('Could not find the requested doctor profile.');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="max-w-4xl mx-auto py-20 px-4">
        <div className="bg-dark-card rounded-lg shadow-xl p-8 md:flex gap-8">
            ...
            <div className="text-center md:text-left mt-6 md:mt-0">
                ...
                <button className="mt-6 bg-brand-blue text-white font-bold py-3 px-8 rounded-lg hover:bg-sky-600 transition-colors">
                    Book Appointment
                </button>
            </div>
        </div>
